...
enum TokenKind {
    ...
    DOUBLE("double"), FINAL("final"), FINALLY("finally"),
    FOR("for"), IMPLEMENTS("implements"), INTERFACE("interface"),
    LONG("long"), SWITCH("switch"), THROW("throw"),
    THROWS("throws"), TRY("try"),

    // Operators
    ASSIGN("="), DEC("--"), EQUAL("=="), GT(">"), INC("++"), LAND("&&"), 
    LE("<="), LNOT("!"), MINUS("-"), PLUS("+"), PLUS_ASSIGN("+="), STAR("*"), 
    DIV("/"), REM("%"), SHL("<<"), SHR(">>"), USHR(">>>"), AND("&"), XOR("^"), OR("|"), NOT("~"),
    SELECT("?"), NEQ("!="), DIV_ASSIGN("/="), SUB_ASSIGN("-="), MUL_ASSIGN("*="),
    REM_ASSIGN("%="), SHR_ASSIGN(">>="), USHR_ASSIGN(">>>="), GE(">="),
    SHL_ASSIGN("<<="), LT("<"), XOR_ASSIGN("^="), OR_ASSIGN("|="), LOR("||"), AND_ASSIGN("&="),
    ...
    STRING_LITERAL("<STRING_LITERAL>"), LONG_LITERAL("<LONG_LITERAL>"),
    DIGITS("<DIGITS>"), EXPONENT("<EXPONENT>"), SUFFIX("<SUFFIX>"),
