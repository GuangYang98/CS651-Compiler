...
public class Parser {
    ...
    private ArrayList<JMember> interfaceBody() {
        ArrayList<JMember> members = new ArrayList<JMember>();
        mustBe(LCURLY);
        while (!have(SEMI) && !see(RCURLY) && !see(EOF)) {
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
            ...
            mustBe(LPAREN);
            JVariableDeclaration forInitDecl = null;
            ArrayList<JStatement> forInit = new ArrayList<JStatement>();
            if (seeLocalVariableDeclaration()) {
                forInitDecl = localVariableDeclarationStatement();
            } else {
            ...
            while (!see(RPAREN) && !see(EOF)) {
                forUpdate.add(statementExpression());
            }
            mustBe(RPAREN);
            JStatement body = statement();
            return new JForStatement(line, forInit, forInitDecl, expr, forUpdate, body);
        } else if (have(WHILE)) {
            JExpression test = parExpression();
            ...
            return new JExceptionStatement(line, tryBlock, parameters, catchBlocks, finalBlock);
        } else if (have(SWITCH)) {
            JExpression test = parExpression();
            mustBe(LCURLY);
            ArrayList<JSwitchBlockStatementGroup> statements = new ArrayList<JSwitchBlockStatementGroup>();
            while (!see(RCURLY) && !see(EOF)) {
    ...
    private JSwitchBlockStatementGroup switchBlockStatementGroup() {
        int line = scanner.token().line();
        boolean def = false;
        ArrayList<JExpression> label = new ArrayList<JExpression>();
        ArrayList<JStatement> body = new ArrayList<JStatement>();
        while (see(CASE) || see(DEFAULT)) {
            JExpression temp = switchLabel();
            if (temp == null) {
                def = true;
                break;
            }
        ...
        while (!see(CASE) && !see(DEFAULT) && !see(RCURLY) && !see(EOF)) {
            body.add(blockStatement());
        }
        return new JSwitchBlockStatementGroup(line, label, body, def);
    }
    ...
    private JExpression assignmentExpression() {
        ...
        if (have(ASSIGN)) {
            ...
            return new JUShrAssignOp(line, lhs, assignmentExpression());
    ...
    private JExpression conditionalExpression() {
        int line = scanner.token().line();
        JExpression testExpression = conditionalOrExpression();
        if (have(SELECT)) {
            JExpression trueExpression = assignmentExpression();
            mustBe(COLON);
            JExpression falseExpression = conditionalExpression();
            testExpression = new JConditionalExpression(line, testExpression, trueExpression, falseExpression);
        }
    ...
    private JExpression relationalExpression() {
        ...
        if (have(GT)) {
            ...
            return new JGreatEqualOp(line, lhs, shiftExpression());
